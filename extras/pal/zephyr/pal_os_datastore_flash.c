...
#define FLASH_PAGE_BUFFER_SIZE                                          \
    (OPTIGA_NVM_DATA_LENGTH > FLASH_WRITE_BLOCK_SIZE                    \
         ? (((OPTIGA_NVM_DATA_LENGTH % FLASH_WRITE_BLOCK_SIZE) == 0)    \
                ...
                : (OPTIGA_NVM_DATA_LENGTH + FLASH_WRITE_BLOCK_SIZE -    \
                   (OPTIGA_NVM_DATA_LENGTH % FLASH_WRITE_BLOCK_SIZE)))  \
         : FLASH_WRITE_BLOCK_SIZE) /* FLASH_WRITE_BLOCK_SIZE is larger than OPTIGA_DATA, we need to buffer the whole block */
...
static pal_status_t prepare_storage_page(void) {
    ...
    for (slot_index = 1; slot_index >= 0; --slot_index) {
        ...
        ret = flash_read(flash_dev, section_offset, &p_page_buffer[0], FLASH_PAGE_BUFFER_SIZE);
        if (ret != 0) {
    ...
    ret = flash_erase(flash_dev, page_offset, FLASH_SECTION_BLOCK_SIZE);
    if (ret != 0) {
        return (PAL_STATUS_FAILURE);
    }
    ...
    ret = flash_write(flash_dev, page_offset, &p_page_buffer[0], FLASH_PAGE_BUFFER_SIZE);
    if (ret != 0) {
        return (PAL_STATUS_FAILURE);
    }
    ...
    if (found_valid_slot) {
        ...
        ret = flash_erase(flash_dev, section_offset, FLASH_SECTION_BLOCK_SIZE);
        if (ret != 0) {
            return (PAL_STATUS_FAILURE);
        }
    }
...
pal_status_t pal_os_datastore_init(void) {
    const struct device* flash_dev = TEST_PARTITION_DEVICE;
...
pal_status_t pal_os_datastore_write(uint16_t id, uint8_t const* buf, uint16_t len) {
    ...
    ret = flash_read(flash_dev, page_offset, &p_page_buffer[0], FLASH_PAGE_BUFFER_SIZE);
    if (ret != 0) {
        return (status);
    }
    ...
    switch (id) {
        case OPTIGA_PLATFORM_BINDING_SHARED_SECRET_ID : {
            if (len <= OPTIGA_SHARED_SECRET_MAX_LENGTH) {
                ...
                p_page_buffer[offset++] = (uint8_t)(len >> 8);
                p_page_buffer[offset++] = (uint8_t)(len);
                memcpy(&p_page_buffer[offset], buf, len);
    ...
    for (int8_t flash_section = 1; flash_section >= 0; --flash_section) {
        ...
        ret = flash_write(flash_dev, section_offset, &p_page_buffer[0], FLASH_PAGE_BUFFER_SIZE);
        if (ret != 0) {
            return (PAL_STATUS_FAILURE);
        }
    }
    ...
    ret = flash_erase(flash_dev, page_offset + FLASH_SECTION_BLOCK_SIZE, FLASH_SECTION_BLOCK_SIZE);
    if (ret != 0) {
        return (status);
    }
...
pal_status_t pal_os_datastore_read(uint16_t id, uint8_t* buf, uint16_t* p_len) {
    ...
    switch (id) {
        case OPTIGA_PLATFORM_BINDING_SHARED_SECRET_ID : {
            ...
            dat_len  = (uint16_t)(p_page_buffer[offset++] << 8);
            dat_len |= (uint16_t)(p_page_buffer[offset++]);
            ...
            if (dat_len > 0) {
                ...
                else if (dat_len <= OPTIGA_SHARED_SECRET_MAX_LENGTH) {
                    memcpy(buf, &p_page_buffer[offset], dat_len);
                    *p_len = dat_len;
                    status = PAL_STATUS_SUCCESS;
                }
                break;
        ...
        case OPTIGA_COMMS_MANAGE_CONTEXT_ID : {
            ...
            if (dat_len > 0) {
                ...
                else if (dat_len <= MANAGE_CONTEXT_BUFFER_SIZE) {
                    memcpy(buf, &p_page_buffer[offset], dat_len);
                    *p_len = dat_len;
                    status = PAL_STATUS_SUCCESS;
                }
        ...
        case OPTIGA_HIBERNATE_CONTEXT_ID : {
            ...
            if (dat_len > 0) {
                ...
                else if (dat_len <= APP_CONTEXT_SIZE) {
                    memcpy(buf, &p_page_buffer[offset], dat_len);
                    *p_len = dat_len;
                    status = PAL_STATUS_SUCCESS;
                }
